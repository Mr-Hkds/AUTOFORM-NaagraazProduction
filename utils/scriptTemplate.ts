...
export const generateAutomationScript = (
    ...
    const serializedConfig = JSON.stringify({
        ...
        delayMax,
        formUrl: cleanUrl,
        appOrigin: typeof window !== 'undefined' ? window.location.origin : 'https://autoform-ai.vercel.app', // Capture app URL
        names,
    ...
    return `
/* 
 * AUTOFORM AI - GOLD EDITION CONTROLLER
 * Generated by AutoForm AI
 * 
 * FEATURES:
...
(async function() {
  ...
  const validateToken = async () => {
    if (!CONFIG.token) {
      ...
      alert('Security Error: This script is missing authentication token. Please regenerate from AutoForm AI.');
      return false;
    }
    ...
    const secret = \`\${userId}-\${timestamp}-autoform-secure-2025\`;
    ...
    try {
      ...
      if (signature !== expectedSignature) {
        ...
        alert('Security Error: This script has been modified and is no longer valid. Please regenerate from AutoForm AI.');
        return false;
      }
    } catch (e) {
  ...
  const createHUD = () => {
    const existing = document.getElementById('autoform-hud');
    ...
    title.textContent = 'AutoForm Gold';
  ...
  if (!runner) {
      ...
      alert('AutoForm: Please allow popups to run the automation.');
      return;
  }
  ...
  try {
      ...
      // Show Success UI
      const hud = document.getElementById('autoform-hud');
      if (hud) {
